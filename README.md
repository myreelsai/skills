...
1. 选择一个可用模型。
2. 调用提交任务接口 `POST /generation/build/:modelName`。
3. 记录返回的 `taskID`。
4. 调用查询接口 `POST /generation/task/:taskID` 轮询状态。
5. 在任务完成后获取结果资源地址。
...
1. 利用するモデルを選択します。
2. `POST /generation/build/:modelName` でタスクを送信します。
3. 返却された `taskID` を保存します。
4. `POST /generation/task/:taskID` で状態をポーリングします。
...
1. Choose a model.
2. Submit a task with `POST /generation/build/:modelName`.
3. Save the returned `taskID`.
4. Poll `POST /generation/task/:taskID` for status updates.
